...
function generatePathNode (node, Doc) {
  var newPathNode = Doc.createElement('path');
...
var fn = function (node, Doc) {
  ...
  switch (tagName) {
    ...
    case 'ellipse': {
      var cx$1 = Number(node.getAttribute('cx'));
      var cy$1 = Number(node.getAttribute('cy'));
      var rx$1 = Number(node.getAttribute('rx'));
      var ry$1 = Number(node.getAttribute('ry'));

      if (!isNaN(cx$1 - cy$1 + rx$1 - ry$1)) {
        path = [
          ("M" + (cx$1 - rx$1) + " " + cy$1),
          ("a" + rx$1 + " " + ry$1 + " 0 1 0 " + (2 * rx$1) + " 0"),
          ("a" + rx$1 + " " + ry$1 + " 0 1 0 " + (-2 * rx$1) + " 0"),
          'z'
        ].join('');
    ...
    case 'polygon':
    case 'polyline': {
      var points = (node.getAttribute('points').match(reNum) || []).map(function (i) { return Number(i); });
...
var XMLSerializer = domParser.XMLSerializer;
var convertShapeToPath = convertShapeToPath$1.fn;
...
var parse = parse_1.parse;
var toSimpleSvg = parse_1.toSimpleSvg;
