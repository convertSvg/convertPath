...
let defaultCenter = false // 默认 viewbox 不居中
...
const pluginList = {
  ...
  viewBoxTransform: (ctx, next) => {
    viewBoxTransform(ctx, defaultSize, defaultCenter)
    next()
  },
 ...
 */
const Dispatcher = (Doc, { plugins = [], size, center }) => {
  defaultSize = 1024 // 重置默认转换 viewbox尺寸
  if (size) defaultSize = size
  if (center) defaultCenter = center
  // 插件队列
  const middleware = []
